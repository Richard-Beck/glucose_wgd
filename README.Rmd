...
saveRDS(y,"data/fitting/fitting_data_v1.Rds")
```
...
wrap_run_old_model <- function(model,gluconc){
  ...
  list(bx=rbind(x2n$bx,x4n$bx),gx=rbind(x2n$gx,x4n$gx))
...
df <- wrap_run_old_model(model,gluconc=gluconc)
...
info <- model_info()
X <- readRDS(paste0("data/fitted_parameters/new_data/opt_",ploidy,"_",model,".Rds"))
v <- X[,"value"]
par <- X[v==min(v),!colnames(X)=="value"]
X <- X[v-min(v)<5,!colnames(X)=="value"]

#df_curve <- do.call(rbind,lapply(1:nrow(X),function(i){
 # pp <- X[i,]
  #names(pp) <- colnames(X)
  #df <- plot_curves(pp,ploidy = ploidy)
  #df$id <- i
  #return(df)
#}))
df_curve <- plot_curves(par,ploidy = ploidy)
df_curve$id <- 1
...
model <- "A3"
source(paste0("Rscripts/models/model",model,".R"))
new_dat <- readRDS("data/fitting/fitting_data_v1.Rds")
new_dat <- melt_data_for_plotting(new_dat)
gpar <- readRDS("data/fitted_parameters/glu_pars.Rds")
