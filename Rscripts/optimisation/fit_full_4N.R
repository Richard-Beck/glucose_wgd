...
y4n <- y4n[sapply(y4n,function(yi) !is.null(yi$gx))]
pars <- readRDS("data/fitted_parameters/init_4N.Rds")
pars["theta"] <- 30000
N0 <- sapply(y4n,function(yi) yi$bx$ncells[yi$bx$metric=="alive"&yi$bx$day==min(yi$bx$day)])
G0 <- sapply(y4n,function(yi) yi$gx$G[1])
names(N0) <- paste0("init_cells",1:length(N0))
names(G0) <- paste0("init_G",1:length(G0))
pars <- c(N0,G0,pars)
pars <- log(abs(pars))
opt4N <- optim(pars,fn=wrap_fit_mod,yxn=y4n)
